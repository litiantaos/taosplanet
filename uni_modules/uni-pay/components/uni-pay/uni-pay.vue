	...
	<view class="uni-pay">
		...
		<uni-popup ref="payConfirmPopup" type="center" :safe-area="false" :animation="false" :mask-click="false">
			<view class="pay-confirm-popup-content">
				...
				<view class="pay-confirm-popup-refresh"><button type="default" @click="_afreshPayment()">支付遇到问题，重新支付</button>
				</view>
		...
		methods: {
			...
			getAppleiapUserName(requestPaymentRes) {
				...
				let info = list.find((item) => {
					return item.transactionIdentifier === requestPaymentRes.transactionIdentifier && item.transactionDate ===
						requestPaymentRes.transactionDate;
				});
